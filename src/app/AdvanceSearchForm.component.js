...
export default class AdvanceSearchForm extends Component {
    constructor(props) {
        ...
        this._onChangeInterpretationText =
            this._onChangeInterpretationText.bind(this);
    ...
    generateAdvSearchText() {
        ...
        if (this.state.dateCreatedFrom)
            summaryStr += `dateCreatedFrom: ${dateUtil.formatDateMMDDYYYY(
                this.state.dateCreatedFrom,
                '/'
            )}, `;
        if (this.state.dateCreatedTo)
            summaryStr += `dateCreatedTo: ${dateUtil.formatDateMMDDYYYY(
                this.state.dateCreatedTo,
                '/'
            )}, `;
        if (this.state.dateModiFrom)
            summaryStr += `dateModiFrom: ${dateUtil.formatDateMMDDYYYY(
                this.state.dateModiFrom,
                '/'
            )}, `;
        if (this.state.dateModiTo)
            summaryStr += `dateModiTo: ${dateUtil.formatDateMMDDYYYY(
                this.state.dateModiTo,
                '/'
            )}, `;
        if (this.state.author.id)
            summaryStr += `author: ${this.state.author.displayName}, `;
        if (this.state.commentator.id)
            summaryStr += `commentator: ${this.state.commentator.displayName}, `;
        if (this.state.interpretationText)
            summaryStr += `interpretationText: ${this.state.interpretationText}, `;
        if (this.state.favoritesName)
            summaryStr += `favoritesName: ${this.state.favoritesName}, `;
        if (this.state.commentText)
            summaryStr += `commentText: ${this.state.commentText}, `;

        // TODO: Need to use Star/Subscribe/Mention..  <-- DOES THIS WORK?
        if (this.state.star) summaryStr += `star: ${this.state.star}, `;
        if (this.state.subscribe)
            summaryStr += `subscribe: ${this.state.subscribe}, `;
        if (this.state.mention)
            summaryStr += `mention: ${this.state.mention}, `;

        if (summaryStr)
            summaryStr = `${otherUtils.advSearchStr}: ${summaryStr.substring(
                0,
                summaryStr.length - 2
            )}`;
        ...
        return (
            <div className="advanceSearchForm">
                <div
                    ...
                    className="btnImages seachPopupCloseImg"
                    role="button"
                    onClick={this._clickCloseBtn}
                >
                    <svg
                        x="0px"
                        y="0px"
                    ...
                    <tbody>
                        <tr>
                            <td className="tdTitle">
                                <span className="searchStyle">Type</span>
                            </td>
                            <td className="tdData">
                                <SelectField
                                ...
                                >
                                    ...
                                    <MenuItem
                                        value="EVENT_REPORT"
                                        primaryText="Event Report Table"
                                    />
                                    <MenuItem value="MAP" primaryText="Map" />
                                </SelectField>
                            </td>
                        </tr>
                        {this.state.showFavoritesNameSearch ? (
                            <tr>
                                <td className="tdTitle">
                        ...
                        <tr>
                            <td className="tdTitle">
                                <span className="searchStyle">Follow-ups</span>
                            </td>
                            <td className="tdData">
                                <table>
                                    <tbody>
