...
const Comment = createReactClass({
    ...
    getInitialState() {
        const comments = this._setComments(
    ...
    _deleteHandler() {
        actions
            .deleteComment(this.props.interpretationId, this.state.data.id)
            .subscribe(() => {
    ...
    _editCommentText() {
        ...
        actions
            .editComment(this.props.interpretationId, this.state.data.id, text)
            .subscribe(() => {
                this.setState(
                    this._setComments(this.state.text, this.state.text)
                )
                const divEditText = `edit_${this.props.data.id}`
                const divShowText = `show_${this.props.data.id}`
    ...
    _cancelCommentText() {
        this.setState(
            this._setComments(this.state.oldText, this.state.oldText),
            function () {
                const divEditText = `edit_${this.props.data.id}`
                const divShowText = `show_${this.props.data.id}`
    ...
    render() {
        ...
        const userName = this.state.data.user.name
            ? this.state.data.user.name.split(' ')
            : ['USER']
                        ...
                        <td>
                            <div className="interpretationComment">
                                <div className="interpretationText">
                                    <div id={divShowText}>
                                        <a className="bold userLink">
                                            {this.state.data.user.name}{' '}
                                        </a>
                                        <span id={divShowContent}>
                                            <RichTextParser>
                                    ...
                                    <div className="hidden" id={divEditText}>
                                        ...
                                        />
                                        <a onClick={this._editCommentText}>
                                            OK
                                        </a>
                                        <label className="linkArea">·</label>
                                        <a onClick={this._cancelCommentText}>
                                            Cancel
                                        </a>
                                    </div>
                                </div>
