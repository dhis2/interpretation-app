  ...
  getSearchTerms(searchTerm) {
    ...
    if (searchTerm !== undefined) {
      ...
      if (searchTerm.idList && searchTerm.idList.length > 0) {
        ...
        if (searchTerm.moreTerms.favoritesName && searchTerm.moreTerms.type)
...
*/
  addToDivList(dataList, hasMore, resultPage) {
    this.setState({
      items: this.state.items.concat([this.createDiv(dataList, resultPage)]),
      hasMore,
    });
  },
      ...
      <div key={divKey}>
        {dataList.map((data) => (
          <Interpretation
            ...
            deleteInterpretationSuccess={this._deleteInterpretationSuccess}
          />
        ))}
      </div>
    );
