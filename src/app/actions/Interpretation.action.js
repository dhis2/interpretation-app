...
actions.listInterpretation.subscribe(
  ({ data: [model, searchData, page], complete, error }) => {
    getD2().then((d2) => {
      let url =
        "interpretations?fields=id,type,text,created,lastUpdated,userGroupAccesses[*]" +
...
actions.getMap.subscribe(({ data: [model, mapId], complete }) => {
  getD2().then((d2) => {
...
actions.getEventReport.subscribe(({ data: [model, reportId], complete }) => {
  getD2().then((d2) => {
...
actions.deleteInterpretation.subscribe(({ data: [model, id], complete }) => {
...
actions.removeLike.subscribe(({ data: [model, id], complete }) => {
  getD2().then((d2) => {
    d2.Api.getApi()
      .delete(`interpretations/${id}/like`)
      .then(complete)
      .catch((errorResponse) => {
        console.log(errorResponse);
      });
...
actions.editInterpretation.subscribe(
  ({ data: [model, id, value], complete }) => {
    getD2().then((d2) => {
      const url = `${d2.Api.getApi().baseUrl}/interpretations/${id}`;

      d2.Api.getApi()
        .request("PUT", url, value, { contentType: "text/plain" })
        .then(complete)
        .catch((errorResponse) => {
          console.log(errorResponse);
        });
